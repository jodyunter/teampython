...
class TestSeriesByWinsRulesService(BaseTestService, TestCase):
    service = SeriesByWinsRulesService()
    ...
    def setup_list(self, session=None):
        ...
        self.service.create("Test Rules A", 4, game_rules1.oid, [1], session)
        self.service.create("Test Rules B", 3, game_rules2.oid, [1, 1], session)
        self.service.create("Test Rules C", 2, game_rules3.oid, [1, 1, 0], session)
        self.service.create("Test Rules D", 1, game_rules1.oid, [1, 1, 0, 0], session)
        self.service.create("Test Rules E", 6, game_rules2.oid, [1, 1, 0, 0, 0], session)
        self.service.commit(session, commit)

    def test_create(self):
        self.setup_test()
        game_rules = self.game_rules_service.create("These game rules", False)
        rules_vm = self.service.create("Test Rules Create", 4, game_rules.oid, [1])
