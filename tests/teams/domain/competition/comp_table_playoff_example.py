...
def setup_config(rand, canadian_league_name, american_league_name, playoff_name, season_game_rules, playoff_game_rules):
    ...
    for key in team_group_map.keys():
        ...
        sub_comp_config = team_group_map[key]["sub_comp"]
        for t in teams:
            team_config = CompetitionTeamConfiguration(t, competition_config, config, 1, None)
            # team_configs.append(team_config)

        # sub_comp_config.competition_group_configs.append(config)
