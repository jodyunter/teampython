...
def create_games(groups, rounds, rules, create_game_method, day_dict):
    for cg in groups:
        ...
        for i in range(rounds):
            new_games = scheduler.schedule_games(cg_teams, rules, 1, 1, True, create_game_method)
            new_days = Scheduler.organize_games_into_days(new_games)
            for day in new_days.keys():
                Scheduler.add_day_to_scheduler(new_days[day], day_dict, 1)
...
all_teams = [calgary, edmonton, toronto, montreal, ottawa, vancouver, quebec_city, winnipeg]
...
print_group(league_config.name, table, "League")
print_group(western_config.name, table, "West")
print_group(eastern_config.name, table, "East")
