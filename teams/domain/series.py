...
class Series(Base, ABC):
    ...
    away_team_from_group_id = Column(String, ForeignKey('competitiongroups.oid'))
    away_team_from_group = relationship("CompetitionGroup", foreign_keys=[away_team_from_group_id])
    away_team_value = Column(Integer)
    winner_to_group_id = Column(String, ForeignKey('competitiongroups.oid'))
    winner_to_group = relationship("CompetitionGroup", foreign_keys=[winner_to_group_id])
    winner_rank_from_id = Column(String, ForeignKey('competitiongroups.oid'))
    winner_rank_from = relationship("CompetitionGroup", foreign_keys=[winner_rank_from_id])
    loser_to_group_id = Column(String, ForeignKey('competitiongroups.oid'))
    loser_to_group = relationship("CompetitionGroup", foreign_keys=[loser_to_group_id])
    loser_rank_from_id = Column(String, ForeignKey('competitiongroups.oid'))
    loser_rank_from = relationship("CompetitionGroup", foreign_keys=[loser_rank_from_id])
